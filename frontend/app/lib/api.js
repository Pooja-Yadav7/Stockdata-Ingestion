export async function fetchDailyClosingPrice() {
  const res = await fetch('http://192.168.49.2:30020/techapp/api/daily-closing-price/');
  if (!res.ok) {
...
export async function fetchTopGainersLosers() {
  const res = await fetch('http://192.168.49.2:30020/techapp/api/top-gainers-losers/');
  if (!res.ok) {
...
export async function fetchPriceChangePercentage() {
  const res = await fetch('http://192.168.49.2:30020/techapp/api/price-change-percentage/');
  if (!res.ok) {
...
export async function fetchDailyStockData() {
  const res = await fetch('http://192.168.49.2:30020/techapp/api/daily-stock-data/');
  if (!res.ok) {
